...
describe('ukBankHolidays', () => {
  ...
  describe('load', () => {
    ...
    it('rejects the promise an error is caught when sending a request to the api', () => {
      const serverError = { value: 500, reasons: 'server error', response: undefined };
      nock('https://www.gov.uk')
        .get('/bank-holidays.json')
        .replyWithError(serverError);

      ukBankHolidays.load().catch(error => {
        expect(error).to.eql(serverError);
      });
    });
    ...
    it('resolves the promise if bankHolidayDates hasn\'t been set but request was successful', async () => {
      ...
      const result = await ukBankHolidays.load();
      expect(result).to.eql(bankHolidayDates);
    });

    it('resolves the promise if bankHolidayDates has already been set', async () => {
      ukBankHolidays.bankHolidayDates = bankHolidayDates;
      const result = await ukBankHolidays.load();
      expect(result).to.eql(ukBankHolidays.bankHolidayDates);
    });
  });

  describe('isDateABankHoliday', () => {
    let isDateInList = null;
